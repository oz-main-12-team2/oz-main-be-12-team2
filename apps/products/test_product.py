...
class AdminViewTest(APITestCase):
    ...
    # 실패테스트
    # test_product.py 파일에 추가

    ## 1. 상품 생성(POST) 실패 로직 테스트
    def test_create_product_fail_with_missing_required_field(self):
        """상품 생성 시 필수 필드(name)를 누락하면 400 에러가 발생하는지 테스트합니다."""

        # 'name' 필드가 없는 유효하지 않은 데이터
        invalid_data = {"description": "이 상품은 이름이 없습니다.", "price": 20000, "stock": 50}

        # API 호출
        response = self.client.post(self.create_url, invalid_data, format="json")

        # 예상대로 400 Bad Request가 오는지 확인
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        # DB에 상품이 생성되지 않았는지 확인
        self.assertEqual(Product.objects.count(), 1)  # setUp에서 만든 1개만 있어야 함

    def test_admin_product_update_success(self):
        """관리자가 상품 정보를 성공적으로 수정하는지 테스트합니다."""
        data = {"name": "수정된 상품", "price": "15000", "stock": 20}
    ...
    # 실패시
    ## 2. 상품 수정(PUT) 실패 로직 테스트
    def test_update_product_fail_with_invalid_data_type(self):
        """상품 수정 시 잘못된 데이터 타입(price)을 보내면 400 에러가 발생하는지 테스트합니다."""

        # 'price' 필드에 숫자 대신 문자열을 넣은 유효하지 않은 데이터
        invalid_data = {
            "name": "잘못된 가격 상품",
            "price": "이건가격이아니야",  # <--- 잘못된 데이터
            "stock": 30,
        }
        ...
        # DB의 상품 정보가 변경되지 않았는지 확인
        self.product.refresh_from_db()  # DB에서 최신 정보 다시 불러오기
        self.assertEqual(self.product.name, "기존 상품")  # 이름이 바뀌지 않았는지 확인
